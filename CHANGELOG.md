...
- `KeyedBatchQueue` allows batching items in a queue around a costly
  operation e.g. calls to `/messages`.
...
- `RoomVersionMirror` for determining if a room version supports _prividlidged creators_.
